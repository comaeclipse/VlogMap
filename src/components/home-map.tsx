...
export function HomeMap({ initialMarkers }: { initialMarkers: Marker[] }) {
  // Initial markers are rendered from the server payload (fallbackData), so pins
  // are present on first paint. SWR revalidates in the background against the
  // slim, cached endpoint.
  ...
  const creators = useMemo(() => {
    ...
    const uniqueCreators = Array.from(
      new Set(data.map((m) => m.creatorName).filter(Boolean)),
    )
